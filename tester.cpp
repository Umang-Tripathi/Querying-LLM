...
int main() {
  ...
  for (int i = 1; i <= num_books; i++) {
    ...
    if (!inputFile.is_open()) {
      std::cerr << "Error: Unable to open the input file mahatma-gandhi."
                << std::endl;
      return 1;
    }
    ...
    while (std::getline(inputFile, tuple, ')') &&
      ...
      qna_tool.insert_sentence(metadata[0], metadata[1], metadata[2],
                               metadata[3], sentence);
    }
  ...
  */
  ...
  while (head != nullptr) {
    string res =
        qna_tool.get_paragraph(head->book_code, head->page, head->paragraph);
    paras.push_back(res);
    head = head->right;
  }
